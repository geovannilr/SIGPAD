        ...
        <td <?php if ($unset_delete) { ?> style="border-left: none;"<?php } ?>
            <?php if ($buttons_counter === 0) {?>class="hidden"<?php }?>>
                <div class="only-desktops"  style="white-space: nowrap">
                    <?php if(!$unset_edit){?>
                        <a data-target = "<?php echo $row->edit_url?>" class="btn btn-default various fancybox.ajax" href="javascript:void(0)"><i class="fa fa-pencil"></i> <?php echo $this->l('list_edit'); ?></a>
                    <?php } ?>
                        ...
                        <?php if ($show_more_button) { ?>
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <?php if (!$unset_read) { ?>
                                        <li>
                                            <a class = "various fancybox.ajax" href="<?php echo $row->read_url?>"><i class="fa fa-eye"></i> <?php echo $this->l('list_view')?></a>
                                        </li>
                                    <?php } ?>
                                    <?php if (!$unset_delete) { ?>
                                        <li>
                            ...
                            <?php } else {
                                if(!empty($row->action_urls)){
                                    foreach($row->action_urls as $action_unique_id => $action_url){
                                        $action = $actions[$action_unique_id];
                                        ?>
                                        <a data-target="<?php echo $action_url; ?>" class="btn btn-default open-window" href="javascript:void(0)">
                                            <i class="fa <?php echo $action->css_class; ?>"></i> <?php echo $action->label?>
                                        </a>
                                    <?php }
                                }
                                ...
                                if (!$unset_read) { ?>
                                    <a class="btn btn-default various fancybox.ajax" href="javascript:void(0)" data-target="<?php echo $row->read_url?>"><i class="fa fa-eye"></i> <?php echo $this->l('list_view')?></a>
                                <?php }
