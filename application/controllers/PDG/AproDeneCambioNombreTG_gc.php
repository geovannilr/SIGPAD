...
class AproDeneCambioNombreTG_gc extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        if($id_cargo_administrativo=='4'){
            ...
            $this->load->model('PDG/AproDeneCambioNombreTG_gc_model');
    ...
    public function _AproDeneCambioNombreTG_output($output = null)
    {

        $data['output'] = $output;
        $data['contenido']='PDG/FormAproDeneCambioNombreTG_gc';
    ...
    {
            $sql="TRUNCATE TABLE pdg_apro_dene_cambio_nombre_temp"; 
            $this->db->query($sql);
            $sql="INSERT INTO pdg_apro_dene_cambio_nombre_temp SELECT * FROM view_apro_dene_cambio_nombre_pdg";
            $this->db->query($sql);

            //obteniendo el año actual
            $fecha_actual=date ("Y"); 
            $crud = new grocery_CRUD();
            $crud->set_table('pdg_apro_dene_cambio_nombre_temp');
           ...
           $crud->display_as('id_solicitud_academica','Id. Solicitud Académica')
            ...
            $crud->callback_update(array($this,'actualizacion_AproDeneCambioNombreTG'));

            $output = $crud->render();

            $this->_AproDeneCambioNombreTG_output($output);
    }
    ...
    public   function actualizacion_AproDeneCambioNombreTG($post_array){
           //Validando el estado
            ...
            $update['estado']=$post_array['estado'];//$estado;
            $update['ruta']=$post_array['ruta'];
            $update['id_tipo_documento']=$id_tipo_documento_pdg;
            
            $comprobar=$this->AproDeneCambioNombreTG_gc_model->Update($update);
            if ($comprobar >=1){
                //$huboError=0;
                if ($update['estado']=='A'){
                    $correo_estudianteA=$this->AproDeneCambioNombreTG_gc_model->ObtenerCorreosIntegrantesEquipo($update['id_equipo_tg'],$update['anio_tg'],$update['ciclo_tg']);
                    $nuevoTema=$this->AproDeneCambioNombreTG_gc_model->ObtenerNuevoTema($update['id_solicitud_academica']);
                    foreach ($correo_estudianteA as $row)
                    {
                            $correoA=$row['email'];
                            //ENVIAR EMAIL a estudiantes que integran el equipo al que se les aprobo el cambio de nombre de tg
                            ...
                            $this->email->subject('Notificación: Aprobacion de Cambio de Nombre de Trabajo de Graduación');
                            $this->email->message('<p>Se ha APROBADO la solicitud de cambio de nombre del Trabajo de Graduación cuyo Tema Antiguo era: '.$post_array['field-tema_tesis'].' por el nuevo nombre de Tema: '.$nuevoTema.'.</p>');
                            $this->email->set_mailtype('html'); 
                            $this->email->send();                        
                    }          
                }    
                if ($update['estado']=='D'){
                    $correo_estudianteD=$this->AproDeneCambioNombreTG_gc_model->ObtenerCorreosIntegrantesEquipo($update['id_equipo_tg'],$update['anio_tg'],$update['ciclo_tg']);
                    foreach ($correo_estudianteD as $row)
                    {
                            $correoD=$row['email'];
                            //ENVIAR EMAIL a estudiantes que integran el equipo al que se les reprobo  el cambio de nombre de tg
                            $this->load->library('email');
                            $this->email->from('sigpa.fia.ues@gmail.com','Sistema Informático para el Control de Procesos Académicos-Administrativos UES');
                            $this->email->to($correoD);
                            $this->email->subject('Notificación: Denegación de Cambio de Nombre de Trabajo de Graduación');
                            $this->email->message('<p>Se ha DENEGADO el Cambio de Nombre de Trabajo de Graduación cuyo Tema es: '.$post_array['field-tema_tesis'].'</p>');
                            $this->email->set_mailtype('html'); 
                            $this->email->send();                        
                    }          
                }     
